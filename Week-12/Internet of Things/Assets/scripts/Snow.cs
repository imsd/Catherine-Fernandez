...
public class Snow : MonoBehaviour {
	...
	public GameObject SnowDrop;				// what object to drop
	public float SnowDropFrequency = 0.5f;	// how often is the drop period
	public int DropMultiplier = 1;	        // how many drops per drop period
	public float DropRadius = 5f;			// 
	private float LastSnowDrop;				// when was the last rain drop

	void Update () {

		// is the current time greater than the last time we dropped rain,
		// plus the rain drop frequency?
		if (Time.time > LastSnowDrop + SnowDropFrequency) {
			...
			for (int i = 0; i < DropMultiplier; i++) {
				...
				// make a rain drop
				GameObject NewDrop = Instantiate (SnowDrop, dropLocation, Quaternion.identity);
